...
import chess.ChessAction.Special;
import chess.ChessBoard;
import chess.ChessBoard.Piece;
...
public class ChessState extends GameState<ChessState, ChessAction> {
    ...
    /**
     * Creates an ChessBoard.empty chess board;
     */
    public ChessState() {
        super(2);
        turn = WHITE;
     ...
     * @param enPassant with column where ChessBoard.enemy pawn just finished a double-advance
    ...
    protected static boolean findKing(ChessBoard board, int turn, Point kingPos) {
        for (int i = 0; i < ChessBoard.DIM; i++) {
            for (int j = 0; j < ChessBoard.DIM; j++) {
                ...
                if (ChessBoard.sameTurn(p, turn) && Piece.valueOf(p) == Piece.King) {
            ...
            for (int j = 0; j < ChessBoard.DIM; j++) {
                ...
                if (!ChessBoard.empty(p)) {
                    score += .01 * Piece.valueOf(p)
                            .getValue(!ChessBoard.sameTurn(p, turn));
                }
            }
        }
    ...
    protected void pawnActions(byte p, int turn,
        ...
        // advance?
        if (ChessBoard.empty(board.get(row + dy, col))) {
            addPawnAction(turn, row, col, row + dy, col, queenRow, as);
            if (row == doubleRow && ChessBoard.empty(board.get(row + dy + dy, col))) {
                as.add(new ChessAction(turn, row, col, row + dy + dy, col));
            }
        }

        // capture?
        if (ChessBoard.enemy(p, board.get(row + dy, col + 1))) {
    ...
    protected void castlingActions(byte p, int turn,
        ...
        // not in inCheck, ChessBoard.empty&non-threatened king transit, ChessBoard.empty rook-transit
        if (!inCheck) {
            if ((canCastle[turn] & CASTLE_SHORT) != 0
                    && ChessBoard.empty(board.get(kingRow, 5))
                    && !threatenedBy(board, other, kingRow, 5)
                    && ChessBoard.empty(board.get(kingRow, 6))
                    && !threatenedBy(board, other, kingRow, 6)) {
                as.add(new ChessAction(turn, row, col,
                        kingRow, 6, Special.ShortCastle));
            }
            if ((canCastle[turn] & CASTLE_LONG) != 0
                    && ChessBoard.empty(board.get(kingRow, 1))
                    && ChessBoard.empty(board.get(kingRow, 2))
                    && !threatenedBy(board, other, kingRow, 2)
                    && ChessBoard.empty(board.get(kingRow, 3))
    ...
    /**
     * Used to generate non-pawn actions. Simply looks for ChessBoard.empty-or-ChessBoard.enemy squares
     * along a given direction, given by row and column deltas. If 'multiple' is se to true,
     * continues looking until non-ChessBoard.empty square found (useful for rook, bishop, queen)
     */
    protected void deltaActions(byte p, int turn,
        ...
        do {
            ...
            if (ChessBoard.empty(target) || ChessBoard.enemy(p, target)) {
                as.add(new ChessAction(turn, row, col, dRow, dCol));
            }
        } while (multiple && ChessBoard.empty(target));
    }
    ...
    protected static boolean canDeltaCapture(ChessBoard b, byte p, int row, int col, int dy, int dx,
        ...
        } while (multiple && ChessBoard.empty(b.get(dRow, dCol)));
        return false;
    }
    ...
    public static boolean threatenedBy(ChessBoard b, int turn, int row, int col) {
        for (int i = 0; i < ChessBoard.DIM; i++) {
            for (int j = 0; j < ChessBoard.DIM; j++) {
                byte p = b.get(i, j);
                if (ChessBoard.sameTurn(p, turn) && canCapture(b, p, i, j, row, col)) {
